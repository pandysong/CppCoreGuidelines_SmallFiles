...
**References**: See the [rules for calling functions](05_F_Functions.md#SS-call).
...
**References**: `Expects()` is described in [GSL](01_main.md#S-gsl).
...
No, using `unsigned` is not a good way to sidestep the problem of [ensuring that a value is nonnegative](09_ES_Expressions_and_statements.md#Res-nonnegative).
...
Better still, use [RAII](08_R_Resource_management.md#Rr-raii) to ensure that the postcondition ("the lock must be released") is enforced in code:
...
**Alternative**: Postconditions of the form "this resource must be released" are best expressed by [RAII](08_R_Resource_management.md#Rr-raii).
...
**See also**: [Generic programming](14_T_Templates_and_generic_programming.md#SS-GP) and [concepts](14_T_Templates_and_generic_programming.md#SS-concepts).
...
* Always carefully [measure](10_Per_Performance.md#Rper-measure) before making performance claims.

**See also**: [I.5](04_I_Interfaces.md#Ri-pre) and [I.7](04_I_Interfaces.md#Ri-post) for reporting precondition and postcondition violations.
...
**Alternative**: [Pass ownership](08_R_Resource_management.md#Rr-smartptrparam) using a "smart pointer", such as `unique_ptr` (for exclusive ownership) and `shared_ptr` (for shared ownership).
...
In that case, mark owning pointers using `owner` from the [guidelines support library](01_main.md#S-gsl):
...
**See also**: [Argument passing](05_F_Functions.md#Rf-conventional), [use of smart pointer arguments](08_R_Resource_management.md#Rr-smartptrparam), and [value return](#Rf-value-return).
...
`not_null` is defined in the [guidelines support library](01_main.md#S-gsl).
...
This violated the rule [against uninitialized variables](09_ES_Expressions_and_statements.md#Res-always),
the rule against [ignoring ownership](04_I_Interfaces.md#Ri-raw),
and the rule [against magic constants](09_ES_Expressions_and_statements.md#Res-magic).
