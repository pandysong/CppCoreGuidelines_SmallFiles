...
The rule [Keep functions short and simple](05_F_Functions.md#Rf-single) implies "Keep loop bodies short."
Similarly, lambdas used as callback arguments are sometimes non-trivial, yet unlikely to be reusable.

##### Enforcement

* See [Keep functions short and simple](05_F_Functions.md#Rf-single)
...
Passing a smart pointer transfers or shares ownership and should only be used when ownership semantics are intended (see [R.30](08_R_Resource_management.md#Rr-smartptrparam)).
...
See further in [R.30](08_R_Resource_management.md#Rr-smartptrparam).
...
* [Prefer `T*` over `T&` when "no argument" is a valid option](05_F_Functions.md#Rf-ptr-ref)
* [Smart pointer rule summary](01_main.md#Rr-summary-smartptrs)
...
* If the function is going to unconditionally move from the argument, take it by `&&`. See [F.18](05_F_Functions.md#Rf-consume).
* If the function is going to keep a copy of the argument, in addition to passing by `const&` (for lvalues),
  add an overload that passes the parameter by `&&` (for rvalues) and in the body `std::move`s it to its destination. Essentially this overloads a "will-move-from"; see [F.18](05_F_Functions.md#Rf-consume).
* In special cases, such as multiple "input + copy" parameters, consider using perfect forwarding. See [F.19](05_F_Functions.md#Rf-forward).
...
* Passing arguments as `T&&` "for efficiency".
  Most rumors about performance advantages from passing by `&&` are false or brittle (but see [F.18](05_F_Functions.md#Rf-consume) and [F.19](05_F_Functions.md#Rf-forward)).
* Returning `const T&` from assignments and similar operations (see [F.47](05_F_Functions.md#Rf-assignment-op).)
...
[That could be dangerous](06_C_Classes_and_class_hierarchies.md#Rc-move-semantic).
...
If you have multiple values to return, [use a tuple](05_F_Functions.md#Rf-out-multi) or similar multi-member type.
...
* To reuse an object that carries capacity (e.g., `std::string`, `std::vector`) across multiple calls to the function in an inner loop: [treat it as an in/out parameter and pass by reference](05_F_Functions.md#Rf-out-multi).
...
In such cases, passing the object by reference [`T&`](05_F_Functions.md#Rf-inout) is usually the right technique.
...
**See also**: [Support library](01_main.md#S-gsl)

**See also**: [Do not pass an array as a single pointer](04_I_Interfaces.md#Ri-array)
...
**See also**: [C.50](06_C_Classes_and_class_hierarchies.md#Rc-factory) regarding when to return a `shared_ptr` from a factory.
...
A reference is often a superior alternative to a pointer [if there is no need to use `nullptr`](05_F_Functions.md#Rf-ptr-ref) or [if the object referred to should not change](???).

##### Note

Do not return a pointer to something that is not in the caller's scope; see [F.43](05_F_Functions.md#Rf-dangle).
...
It's asking to return a reference to a destroyed temporary object. A `&&` is a magnet for temporary objects. This is fine when the reference to the temporary is being passed "downward" to a callee, because the temporary is guaranteed to outlive the function call (see [F.18](05_F_Functions.md#Rf-consume) and [F.19](05_F_Functions.md#Rf-forward)). However, it's not fine when passing such a reference "upward" to a larger caller scope. See also ???.
...
[Default arguments for virtual functions](06_C_Classes_and_class_hierarchies.md#Rh-virtual-default-arg)
