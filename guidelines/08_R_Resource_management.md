...
Where a resource is "ill-behaved" in that it isn't represented as a class with a destructor, wrap it in a class or use [`finally`](12_E_Error_handling.md#Re-finally)

**See also**: [RAII](08_R_Resource_management.md#Rr-raii)
...
In addition to suffering from the problem from [leak](#???), this adds a spurious allocation and deallocation operation, and is needlessly verbose. If Gadget is cheap to move out of a function (i.e., is small or has an efficient move operation), just return it "by value" (see ["out" return values](05_F_Functions.md#Rf-out)):
...
**See also**: [The raw pointer rule](08_R_Resource_management.md#Rr-ptr)

##### Enforcement

See [the raw pointer rule](08_R_Resource_management.md#Rr-ptr)
...
Both cases are an error under the [`sharedptrparam` guideline](08_R_Resource_management.md#Rr-smartptrparam):
