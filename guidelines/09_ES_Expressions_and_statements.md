...
**See also**: [ES.28](09_ES_Expressions_and_statements.md#Res-lambda-init)
...
For initializers of moderate complexity, including for `const` variables, consider using a lambda to express the initializer; see [ES.28](09_ES_Expressions_and_statements.md#Res-lambda-init).
...
Avoid macros if you can: [ES.30](09_ES_Expressions_and_statements.md#Res-macros), [ES.31](09_ES_Expressions_and_statements.md#Res-macros2), and [ES.32](09_ES_Expressions_and_statements.md#Res-ALL_CAPS).
...
(left-to-right except right-to-left in assignments, and the order of evaluation of function arguments is unspecified; [see ES.43](09_ES_Expressions_and_statements.md#Res-order)),
...
Pointers should [only refer to single objects](04_I_Interfaces.md#Ri-array).
...
This rule is part of the [bounds-safety profile](21_Pro_Profiles.md#SS-bounds).
...
* Flag `const_cast`s.
* This rule is part of the [type-safety profile](21_Pro_Profiles.md#Pro-type-constcast) for the related Profile.
...
A move typically leaves behind an empty object ([C.64](06_C_Classes_and_class_hierarchies.md#Rc-move-semantic)), which can be surprising or even dangerous, so we try to avoid moving from lvalues (they might be accessed later).
...
And after you do that, assume the object has been moved from (see [C.64](06_C_Classes_and_class_hierarchies.md#Rc-move-semantic)) and don't read its state again until you first set it to a new value.
...
**See also**: [R: Resource management](01_main.md#S-resource)
...
This example not only violates the [no naked `new` rule](09_ES_Expressions_and_statements.md#Res-new) as in the previous example, it has many more problems.
...
The first defense against this is to [define the base class `Shape` not to allow this](06_C_Classes_and_class_hierarchies.md#Rc-copy-virtual).
...
[Avoid casts](09_ES_Expressions_and_statements.md#Res-casts) (explicit type conversion) and if you must [prefer named casts](09_ES_Expressions_and_statements.md#Res-casts-named).
...
The construction notation is the most general [initializer notation](09_ES_Expressions_and_statements.md#Res-list).
...
This is a major part of the discussion of [C++'s resource- and type-safety model](01_main.md#Stroustrup15).

**See also**:

* Use [RAII](08_R_Resource_management.md#Rr-raii) to avoid lifetime problems.
* Use [unique_ptr](05_F_Functions.md#Rf-unique_ptr) to avoid lifetime problems.
* Use [shared_ptr](05_F_Functions.md#Rf-shared_ptr) to avoid lifetime problems.
* Use [references](05_F_Functions.md#Rf-ptr-ref) when `nullptr` isn't a possibility.
* Use [not_null](#Rf-not_null) to catch unexpected `nullptr` early.
* Use the [bounds profile](21_Pro_Profiles.md#SS-bounds) to avoid range errors.
...
This rule is part of the [lifetime safety profile](21_Pro_Profiles.md#SS-lifetime)
...
**See also**: [Don't use a variable for two unrelated purposes](09_ES_Expressions_and_statements.md#Res-recycle)
...
If you really need to break out a loop, a `break` is typically better than alternatives such as [modifying the loop variable](09_ES_Expressions_and_statements.md#Res-loop-counter) or a [`goto`](09_ES_Expressions_and_statements.md#Res-goto):
...
This precludes consistency. Use `gsl::index` for subscripts; [see ES.107](09_ES_Expressions_and_statements.md#Res-subscripts).
...
This makes surprises (and bugs) inevitable.
 ...
 Use `gsl::index` for subscripts; [see ES.107](09_ES_Expressions_and_statements.md#Res-subscripts).
...
Consider:
    ...
    unsigned area(unsigned height, unsigned width) { return height*width; } // [see also](04_I_Interfaces.md#Ri-expects)
    // ...
    int height;
    cin >> height;
